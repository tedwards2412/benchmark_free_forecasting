...
def Calclims():
    ...
    for m in mlist:
        dRdS = np.array(dRdS1(s1means, m, epsilon=millinorm, millicharge=True)*s1width)
        UL1T = SF.upperlimit(dRdS, 0.1)
        UL100T = SF2.upperlimit(dRdS, 0.1)
        ULlist_Xenon1T_milli.append(UL1T*millinorm**2)
        ULlist_Xenon100T_milli.append(UL100T*millinorm**2)
...
def Euclideanizemilli(nsamples = 100000):
    ...
    for m in tqdm(ms, desc="Euclideanizing Xenon-nT"):
        ...
        for epsilon in coupling_temp:
            couplings.append([m,epsilon,0.])
            dRdS, Nsig = np.array(dRdS1(s1means, m, epsilon=epsilon, millicharge=True, Nevents=True, sigmav=sigmav[i], vesc=vesc[i], vlag=vlag[i]))
    ...
    for i, m in enumerate(tqdm(couplings[:,0], desc="Euclideanizing DS20k")):
            m = couplings[i,0]
            epsilon = couplings[i,1]

            dRdE = dRdEAr(m, ER_c, eff_temp, epsilon=epsilon, millicharge=True, sigmav=sigmav[i], vesc=vesc[i], vlag=vlag[i])
...
#######################
    ...
    # Output to new hdf5
    outfile = "../hdf5/Xenon100T_DS20k_gridscanmillicharge_HaloTrue.hdf5"
